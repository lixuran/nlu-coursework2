...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # the posion embedding is used to mark the posion of each token in the sentence and it is combined with the sentence embedding so that the model can learn the relative order of each word
        # we need this position embeddings as attention layers are permutation equivariant, which means that without explict differentiation through hard coded positioning, sentences with the same group of words
        # and different ordering would result in the same output.
        ...
        #print("src_out ",forward_state.shape)
        #print("src_embeddings ", src_embeddings.shape)
        return {
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        #print("tgt_inputs ",tgt_inputs.shape)
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            '''
            # the self_attn_mask is used to mask out the parts of the embedding in the forward_state where its timestamp is larger than the
            # current target word. in particular, it is implemented as a matrix with the same dimensionality as the original forward_state and
            # the lower half triangle is filled with negative infinity. The decoder need this mask so that the model can not look forward into the sentence
            # when predicting the next word, while since the encoder task is simply to represent the input sequence so no such mask is needed.
            # for incremental decoding,as we are only given the current word and its previous word, the mask can not be applied in the same manner.
            # this incremental decoding can be treated as using attention mechanism to learn a n-gram language model with n=2 , so in this case the mask is not needed to
            # mask out the previous word
        ...
        if not features_only:
            ...
            '''
            #the linear projection project the hidden output tensor into the word embedding space. after this line
            # the dimensionality of forward_state should equal the length of the dictionary for English sentences, which is 4417
            # the output would represent the output hidden tensor given by the decoder that contains the learned features of the corresponding
            # sentence so far.
