...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            #compute the model output on the current batch of samples,use it to compute loss by comparing with the target tokens according to specified criterion
            # and compute the gradient for all parameters.
            #if the norm of the sum of the gradient is larger then clip_norm,apply normalizaiton to the norm = clip_norm. then update the parameters and clear the grads for the next update.
            loss = \
