...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # when bidirectional is set to true, the the size of the hidden states and cell states are doubled, and the states for the farword and backword pass of the
        # model are stored in a interleaving way,.i.e. ABABAB..., and the model reform the hidden states and cell states so that the states generated by the forward pass of the sentence
        # is stored using the first half of the first dimension of the state matrix.

        # final_hidden_states is the matrix which stores the final hidden embedding at t= src_time_steps which was used previously to update the next timestamp, and the final cell state is the
        # "true " hidden state that the final lstm cell contains.
        if self.bidirectional:
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        #the attention context vector is calculated using a weighted average of the source hidden vector, i.e. the encoder_out.
        #the weight of the encoder_out vector is stored in attn_weights, which is calculated by applying a mask on the original attention score function
        #defined over the target word tgt_input and the source embedding

        # we apply a mask to the attention score in order to zero out the attention weight of the embedding of the words that comes after the target word
        # in the original sentence so that the model would not learn to make a simply copy of the next input embedding to make a prediction in the decoder.
        ...
        #attn_out is the output of probability of each word using attention
    ...
    def score(self, tgt_input, encoder_out):
        ...
        # tgt_input has shape = [batch_size, input_dims]
        # it is unknown if there s a mistake in src_projection function
        # as it currently takes a vector of dim input_dims and output a vector of shape output_dims
        # while encoder_out last dimension is of size output_dims, and it can be inferred from the shape requirement of torch.bmm
        # that the output dimension of src_projection should be input_dims. maybe output_dims = input_dims?
        # assuming this is true, then we have projected_encoder_out shape:[batch_size.input_dims,src_time_steps]
        # after unsqueeze, tgt_input became a vector of shape [batch_size, 1,input_dims]
        # applying batch matrix multiplication as in torch.bmm, we obtain the attention scores with shape [batch_size,1,src_time_steps]
        # which is percisely a batch-wise multiplication of current output hidden state and the embedding of the input sequence given by the encoder
        # which results in attn_scores having the shape [batch_size, 1, src_time_steps]
        # bmm makes sure the both encoder and decoder representation have the same shape for embeddings.
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.lexical_layer= nn.Linear(embed_dim,embed_dim, bias=False)
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        #print("shape of source embedding ",src_embeddings.shape)
        #print(src_embeddings.transpose(0,1).shape)
        ...
        '''
        # the decoder state is initialized by first checking if there is any cached state in previous round if incremental mode is selected, and create two
        # list of matrix filled with zeroes of shape [tgt_inputs.size()[0], self.hidden_size, self.layers], and a empty input feed if incremental_state is None .
        # the cached state is none if incremental_state is None or the _fairseq_instance_id of the decoder model is not in incremental_state
        # the input_feed is combined with the current hidden state in the forward pass to give prediction and update the hidden state at
        #each time step.
        ...
        for j in range(tgt_time_steps):
            ...
            context = torch.zeros([batch_size, src_embeddings.shape[2]])
            ...
            '''
            # if the attention is used to predict the next output,
            # the last target state, the output embedding given by the encoder,i.e. src_out and a mask is provided to the attention function in order to
            # obtain the attentioned weighted input embedding and the attention weights. the previous target state is used to compute the attention score, as it
            #indicates the current position and meaning in the sentence to be produced by the decoder and thus indicates which part of the original sentence should
            # obtain more "attention"
            #the dropout layer is applied during training to randomly zero out a portion of the attention weighted input so that the training process is less prone to
            #overfit to perticular training input pattern.
            if self.attention is None:
                input_feed = tgt_hidden_states[-1] #here the input feed is already the output hidden embedding
            else:
                ...
                if self.use_lexical_model:
                    ...
                    # TODO: --------------------------------------------------------------------- CUT
                    #step_attn_weights #shape [batch_size, src_time_steps]
                    #src_embeddings # shape [src_time_steps, batch_size, num_features]
                    #print(step_attn_weights.shape, src_embeddings.shape)

                    #print(step_attn_weights.shape,src_embeddings.shape)
                    #print()
                    for index in range(src_time_steps):
                        for b_index in range(batch_size):
                            context[b_index,:]+=step_attn_weights[b_index,index]*src_embeddings[index,b_index,:]
                         #TODO: this might be wrong

                    context=torch.tanh(context)
                    #print("shape of lexical context ",context.shape)
                    context2=torch.tanh(self.lexical_layer(context))+context

                    lexical_contexts.append(context2) # this should be list of [batch_size,num_features]
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            #lexical_contexts=torch.tensor(lexical_contexts)
            #print("final lexical contexts shape ",len(lexical_contexts))
            #print(torch.cat(lexical_contexts).shape)
            #print(decoder_output.shape)
            decoder_output=decoder_output+self.final_lexical_projection(torch.cat(lexical_contexts).transpose(0,1))
